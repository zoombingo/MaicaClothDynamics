...
namespace ClothDynamics
{
    ...
    public class SkinnerSource : MonoBehaviour
    {
        ...
        public SkinnerModel _model;

        public Mesh _cacheSkinnedMesh;
        ...
        [SerializeField] RenderTexture _positionBuffer0;
        [SerializeField] RenderTexture _positionBuffer1;
        [SerializeField] RenderTexture _normalBuffer;
        [SerializeField] RenderTexture _tangentBuffer;
        ...
        RenderTexture CreateBuffer()
        {
            ...
            var sqrtSize = Mathf.CeilToInt(Mathf.Sqrt(_model.vertexCount));

            //var rt = new RenderTexture(_model.vertexCount, 1, 0, format); //This works with skinner effect shaders
            var rt = new RenderTexture(sqrtSize, sqrtSize, 0, format);
        ...
        void BuildCamera()
        {
            ...
            var go = new GameObject("SkinnerCamera");
        ...
        public static bool CheckSkinned(Mesh mesh)
        {
        ...
        void CheckIfMeshExists()
        {
            var source = GetComponent<SkinnedMeshRenderer>().sharedMesh;// GetMeshPath(AssetDatabase.GetAssetPath(smr.sharedMesh), smr);
            if (CheckSkinned(source))
            {
                SkinnerModel asset = ScriptableObject.CreateInstance<SkinnerModel>();
                asset.Initialize(source);

#if UNITY_EDITOR
                var dirPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath(source));
                var assetPath = AssetDatabase.GenerateUniqueAssetPath(dirPath + "/" + asset.mesh.name + " Skinner Model.asset");
                ...
                _model = asset;
//#if UNITY_EDITOR
//                Debug.Log("You need to create a SkinnerModel for the Asset located in \"" + dirPath + "\" with the mesh named \"" + source.name + "\" !\nRight click on the mesh and select Skinner->Convert Mesh");
//                StartCoroutine(DelayedAssetStoring(source, asset));
//#endif
            }
        }
        ...
        private IEnumerator SaveModelPath()
        {
            while (_model == null)
                yield return null;
            var modelPath = AssetDatabase.GetAssetPath(_model);
            PlayerPrefs.SetString(this.GetInstanceID().ToString(), modelPath);
            PlayerPrefs.Save();
        }
        ...
        public void OnPlaymodeChanged(PlayModeStateChange state)
        {
            ...
            {
                StartCoroutine(SaveModelPath());
            }
            else if (state != PlayModeStateChange.EnteredPlayMode && !EditorApplication.isPlayingOrWillChangePlaymode)
            {
                var modelPath = PlayerPrefs.GetString(this.GetInstanceID().ToString());
                var model = AssetDatabase.LoadAssetAtPath<SkinnerModel>(modelPath);
                if (model != null)
                    _model = model;
            }
        }
        ...
        private void Awake()
        {
            ...
            if (_model == null) CheckIfMeshExists();
        }
        ...
        void Start()
        {
            ...
            smr = GetComponent<SkinnedMeshRenderer>();
            _cacheSkinnedMesh = new Mesh();
            smr.BakeMesh(_cacheSkinnedMesh);
        ...
        void LateUpdate()
        {
            ...
            Shader.SetGlobalFloat("_skinnerVertexTexWidth", _positionBuffer0.width);
