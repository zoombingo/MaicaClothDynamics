...
namespace ClothDynamics
{
    ...
    [DefaultExecutionOrder(15160)] //When using Final IK
    ...
    public class SkinnerDebug : MonoBehaviour
    {
        ...
        void Start()
        {
            if (source == null) source = this.GetComponent<SkinnerSource>();
            ...
            //int sqrtLinesPerMesh = Mathf.CeilToInt(Mathf.Sqrt(linesPerMesh));

            for (var i = 0; i < linesPerMesh; i++)
            {
                var vi = i * 6;
                //int n = i / sqrtLinesPerMesh;
                var u = (0.5f + i) / linesPerMesh;
                var v = 0;// (0.5f + n) / sqrtLinesPerMesh;
                vertices[vi + 0] = new Vector3(u, v, 0); // velocity line
                vertices[vi + 1] = new Vector3(u, v, 1);
                vertices[vi + 2] = new Vector3(u, v + 1, 0); // normal line
                vertices[vi + 3] = new Vector3(u, v + 1, 1);
                vertices[vi + 4] = new Vector3(u, v + 2, 0); // tangent line
                vertices[vi + 5] = new Vector3(u, v + 2, 1);
            }
        ...
        void LateUpdate()
        {
            ...
            {
                var bufferOffset = i;// (float)i / (linesPerMesh * drawCount);
                //Debug.Log(bufferOffset);
                _props.SetFloat("_BufferOffset", bufferOffset);
                Graphics.DrawMesh(
