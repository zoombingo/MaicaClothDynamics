...
v2f vert(appdata v)
{
    ...
    uint vertexID = _BufferOffset * 10922 + uvw.x * 10922;
    float4 texcoord = float4(((vertexID % uint(_skinnerVertexTexWidth)+ 0.5)/(_skinnerVertexTexWidth)), ((vertexID / uint(_skinnerVertexTexWidth)+ 0.5)/(_skinnerVertexTexWidth)), 0, 0);
