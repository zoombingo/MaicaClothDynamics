...
// Seed for PRNG
float _RandomSeed;
float _skinnerVertexTexWidth;

// PRNG function
float UVRandom(float2 uv, float salt)
{
    //uint vertexID = (uv.x) * 10922; //
    //uv = float2(((vertexID % uint(_skinnerVertexTexWidth)+ 0.5)/(_skinnerVertexTexWidth)), ((vertexID / uint(_skinnerVertexTexWidth)+ 0.5)/(_skinnerVertexTexWidth)));
