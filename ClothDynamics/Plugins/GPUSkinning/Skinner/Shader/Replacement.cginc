...
v2f vert(appdata v)
{
    ...
    o.position = float4(((v.vertexID % uint(_skinnerVertexTexWidth) + 0.5)/(_skinnerVertexTexWidth)) * 2 - 1, ((v.vertexID / uint(_skinnerVertexTexWidth) + 0.5)/(_skinnerVertexTexWidth)) * 2 - 1, 0, 1);
#if defined(SKINNER_POSITION)
